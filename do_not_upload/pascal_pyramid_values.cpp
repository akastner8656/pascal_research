...
int main()
{
	...
	if (answer == "Y" || answer == "y")
	{
		...
		for (triangleNumber = 1; triangleNumber <= numberTrianglesWanted; triangleNumber++)
		{
			...
			for (rowNumber = 0; rowNumber < rowsProduced; rowNumber++) 						    //The for loop is used to create a triangle of values.
			{
				...
				for (colNumber = 0; colNumber <= rowNumber; colNumber++)
				{
					...
					else
					{
						...
						{
							katieTriangleMultiplier = (rowCoordinate - y + 1) / (y);			//katieTriangleMultiplier is used to create the next value in any given row.
							multiplier = multiplier * katieTriangleMultiplier;
							pascalMultiplier = multiplier; 										//pascalMultiplier will take values from Pascal's triangle and multiply them by the main equation.
						}
						multiplier = 1;
					}
